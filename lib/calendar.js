...
function toEventRecord (title, start, end, isFullDay) {
...
function expandEventInstances (entry, nowMs) {
	...
	try {
		return nodeIcal.expandRecurringEvent(entry, {
...
function parseEvents (parsed, nowMs = Date.now()) {
	...
	for (const key of Object.keys(parsed)) {
		...
		for (const instance of instances) {
			...
			const end = instance.end ? new Date(instance.end) : start;
...
/**
 * Fetch a URL with manual redirect handling so each hop is SSRF-checked.
 */
async function fetchWithValidatedRedirects (urlString, fetchOptions = {}, allowedHosts, allowedSuffixes) {
	let currentUrl = urlString;

	for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
		const allowed = isUrlAllowed(currentUrl, allowedHosts, allowedSuffixes);
		if (!allowed.ok) {
